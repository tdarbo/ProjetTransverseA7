...
class PlayScene(Scene):
    ...
    def select_maps(self, count):
        ...
        selected_maps = random.sample(maps, count)
    ...
    def on_enter(self):
        ...
        self.selected_maps = self.select_maps(holes_number)
        self.levels_played = 0
    ...
    def load_level(self):
        ...
        if self.levels_played < len(self.selected_maps):
            self.reset_players()
            map_info = self.selected_maps[self.levels_played]
            self.current_level = self.create_level(self.levels_played, map_info)
            self.score_manager.set_current_hole(self.levels_played)
            print(f"Niveau {self.levels_played + 1} chargé.")
